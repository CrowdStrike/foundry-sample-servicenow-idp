...
echo -e "${YELLOW}Temporarily disabling workflow auto-provisioning...${NC}"
# Use sed differently on macOS vs Linux
if [[ "$OSTYPE" == "darwin"* ]]; then
  sed -i '' 's/provision_on_install: true/provision_on_install: false/g' workflows/*.yml
else
  sed -i 's/provision_on_install: true/provision_on_install: false/g' workflows/*.yml
fi
echo -e "${GREEN}✓ Modified workflow files${NC}"

echo -e "${YELLOW}Deploying app with Foundry CLI...${NC}"
foundry apps deploy --change-type=major --change-log="E2E test deployment with disabled workflow provisioning"
